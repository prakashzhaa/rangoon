#!/bin/bash
: '
@name   rangoon rangoon.sh
@author prakashzhaa
@link   https://github.com/prakashzhaa/rangoon
'
...
displayLogo() {
    ...
    ||  \/\    ||	|| ||	   \||  ||  ||  ||  ||    ||  ||    ||	||      \||
    ||   \/\   ||	|| ||	    ||   ||||	||   ||||||    ||||||	||       ||                                                 
                                          v$VERSION - $YELLOW@prakashzhaa$RESET" 
		}
...
gatherSubdomains() {
	...
	"$HOME"/go/bin/subfinder -d "$domain" -all -config "$HOME"/rangoon/configs/config.yaml -o "$SUBS"/subfinder.txt
	echo -e "[$GREEN+$RESET] Done, next."
	...
	startFunction "amass"
	"$HOME"/go/bin/amass enum -passive -d "$domain" -config "$HOME"/rangoon/configs/config.ini -o "$SUBS"/amassp.txt
	echo -e "[$GREEN+$RESET] Done, next."
...
gatherIPs() {
	...
	python3 $HOME/rangoon/scripts/clean_ips.py "$IPS"/"$domain"-ips.txt "$IPS"/"$domain"-origin-ips.txt
	echo -e "[$GREEN+$RESET] Done."
}
...
portScan() {
	...
	cat "$SUBS"/alive_subdomains | naabu -p - -silent -no-probe -exclude-cdn -nmap -config "$HOME"/rangoon/configs/naabu.conf
    mv rangoon-nmap* "$PORTSCAN"
...
notifySlack() {
	...
	source "$HOME"/rangoon/configs/tokens.txt
	export SLACK_WEBHOOK_URL="$SLACK_WEBHOOK_URL"
	echo -e "rangoon $domain scan completed!" | slackcat
...
notifyDiscord() {
	...
	python3 $HOME/rangoon/scripts/webhook_Discord.py <<< $(echo "$message")
...
source "$HOME"/rangoon/configs/tokens.txt || return
export SLACK_WEBHOOK_URL="$SLACK_WEBHOOK_URL"
