#!/bin/bash
: '
@name   Rangoon install.sh
@author prakash zhaa
@link   https://github.com/prakash/rangoon
'
...
displayLogo() {
	...
	: 'Basic requirements'
	basicRequirements() {
		echo -e "[$GREEN+$RESET] This script will install the required dependencies to run rangoon.sh, please stand by.."
...
golangInstall() {
	...
	if [ "$(cat "$configfile" | grep '^alias recon=')" == "" ]; then
		echo "alias recon=$HOME/rangoon/rangoon.sh" >>"$HOME"/.bashrc
	fi
